...
export class ServiciosDispController {
    ...
    readServicioGuarderia() {
        ...
        let guarderias = [];
        return this.firebaseReadRepository.readGroupCollection("guarderiasusuario").get().then(
    ...
    readServicioPaseo() {
        let promedio = this.promedio
        let paseos = [];
        return this.firebaseReadRepository.readGroupCollection("paseosusuario").get().then(
            function (querySnapshot) {
                querySnapshot.forEach(function (doc) {
                    let id = { "id": doc.id }
                    let values = { ...doc.data(), ...id }
    ...
    readServicioVeterinaria() {
        let promedio = this.promedio
        let veterinarias = [];
        return this.firebaseReadRepository.readGroupCollection("veterinariasusuario").get().then(
            function (querySnapshot) {
                querySnapshot.forEach(function (doc) {
                    let id = { "id": doc.id }
                    let values = { ...doc.data(), ...id }
